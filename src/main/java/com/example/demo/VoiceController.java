...
public class VoiceController {
	...
	public String sendVoice(String msg, String lang, HttpServletRequest request)
	{
		...
		String path=request.getSession().getServletContext().getRealPath("/");//webapp에 저장됨
		 ...
		 String clientId = "b34x7qvvr7";//애플리케이션 클라이언트 아이디값";
	        String clientSecret = "FoJwdtqwWyoSD8hkkzIUWqJdzrWnw1ItQZN6vFh3";//애플리케이션 클라이언트 시크릿값";
	        try {
	            ...
	            if(responseCode==200) { // 정상 호출
	                ...
	                File f = new File(path+"/"+tempname + ".mp3");//webapp에 mp3파일 저장
	                f.createNewFile();
